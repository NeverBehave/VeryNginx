...
点击下方按钮在 PWD 上快速启动一个项目并体验！

___默认暴露80，点击面板上“80”即可访问___

默认密码： verynginx/verynginx

[![Try in PWD](https://cdn.rawgit.com/play-with-docker/stacks/cff22438/assets/images/button.png)](http://play-with-docker.com?stack=https://raw.githubusercontent.com/NeverBehave/VeryNginx/master/stack.yml?v1)

目录
=================

* [介绍](#介绍)
    * [Nginx 运行状态分析](#Nginx-运行状态分析)
    * [自定义行为](#自定义行为)
        * [Matcher](#matcher)
        * [Action](#action)
        * [Backend](#backend)
    * [访问统计](#访问统计)
* [安装和使用说明](安装和使用说明)
    * [安装 VeryNginx](#安装-verynginx)
        * [使用打包好的 Docker 镜像](#使用打包好的-docker-镜像)
        * [想使用自己的 Nginx?](#想使用自己的-nginx)
    * [使用](#使用)
        * [编辑 Nginx 配置文件](#编辑-nginx-配置文件)
        * [启动/停止/重启 服务](#启动停止重启-服务)
    * [故障排除](#故障排除)
        * [提示](#提示)
        * [详细的配置说明](#详细的配置说明)
    * [更新 VeryNginx ／ OpenResty](#更新-verynginx--openresty)
* [捐赠](#捐赠)
...
```bash
git clone https://github.com/alexazhou/verynginx/ --depth=1
cd verynginx
...
```bash
docker run -d --restart=unless-stopped -p 80:80 neverbehave/verynginx 
```

如果需要持久化配置文件到宿主机：

```bash
docker run -d --restart=unless-stopped -p 80:80 -v ${YOUR_CONFIG_PATH}:/opt/verynginx/verynginx/configs neverbehave/verynginx
```

**注意！** 由于挂载到外部目录会覆盖容器内的权限设定，你可能需要手动在宿主机执行以下命令

```bash
chmod -R 777 ${YOUR_CONFIG_PATH}
```

> 镜像基于 [docker openresty](https://github.com/openresty/docker-openresty/blob/master/alpine/Dockerfile)
...
本 REPO 为 FORK，请移步原作者 REPO： https://github.com/alexazhou/VeryNginx
